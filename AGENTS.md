...
- **API**: define routes in `src/lib/api-routes.ts` (typed Hono chain); validate bodies with a
  `hono/validator` wrapping zod `safeParse` — at the door, not inside the handler, or the client
  can't send a body — give every response an explicit status, no `as`/`unknown` casts →
